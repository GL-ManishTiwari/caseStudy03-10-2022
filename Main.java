...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        if (numItemPurchase <= 0) {
            ...
            OrderedItem[] ordItem = new OrderedItem[numItemPurchase];
            for (int i = 0; i < numItemPurchase; i++) {
                ...
                if (ItemData.isAvailable(itemName)) {
                    ...
                    ordItem[i] = new OrderedItem(itemName, rate, itemQty, floatToString(discountAmount),
            ...
            for (int i = 0; i < numItemPurchase; i++) {
                String name = ordItem[i].getItemName();
                String rate = ordItem[i].getRate();
                String qty = ordItem[i].getOrderQty();
                Double price = Double.parseDouble(ordItem[i].getRate()) * Double.parseDouble(qty);
                String dis = ordItem[i].getDiscountAmount();
                String amount = ordItem[i].getPayAmount();
